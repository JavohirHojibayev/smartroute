...
export type TurnstileDailyAccessKpis = {
  ...
  flaggedToday: number;
  /** Dashboard telemetriya oynalari: dedupe qilingan haqiqiy o‘tishlar */
  passMovementRowsDeduped: AccessLog[];
};

/**
 * @param accessRowsToday — `access_time` berilgan `bounds` oralig‘idagi qatorlar (GET /summary jurnal sanalari bilan mos)
 * @param bounds — `start` inklyuziv, `end` eksklyuziv (`getAccessKpisTashkentDayBounds()` yoki jurnal oralig‘i + 1 ms)
 */
export function computeTurnstileDailyAccessKpis(
  ...
  const zavodEntrancesToday = passMovementRowsDeduped.filter((row) => resolveTurnstileMovementType(row) === 'entrance' && !isMineShahtaFromStoredDevices(row.device_id, row.device_name)).length;
  const zavodExitsToday = passMovementRowsDeduped.filter((row) => resolveTurnstileMovementType(row) === 'exit' && !isMineShahtaFromStoredDevices(row.device_id, row.device_name)).length;
  const mineEntrancesToday = passMovementRowsDeduped.filter((row) => resolveTurnstileMovementType(row) === 'entrance' && isMineShahtaFromStoredDevices(row.device_id, row.device_name)).length;
  const mineExitsToday = passMovementRowsDeduped.filter((row) => resolveTurnstileMovementType(row) === 'exit' && isMineShahtaFromStoredDevices(row.device_id, row.device_name)).length;

  return { entrancesToday, exitsToday, zavodEntrancesToday, zavodExitsToday, mineEntrancesToday, mineExitsToday, flaggedToday, passMovementRowsDeduped };
}

...
function Resolve-DepartmentFromUser {
    param($User)
    if ($null -eq $User) { return $null }

    $candidateKeys = @(
        "department", "departmentName", "dept", "deptName",
        "division", "divisionName", "org", "orgName",
        "organization", "organizationName", "unit", "unitName",
        "group", "groupName", "team", "teamName", "section", "sectionName", "workshop", "workshopName"
    )

    foreach ($key in $candidateKeys) {
        if ($User.PSObject.Properties.Name -contains $key) {
            $value = $User.$key
            $normalized = Normalize-Department -Value ([string]$value)
    ...
    foreach ($property in $User.PSObject.Properties) {
        ...
        if ($value -is [pscustomobject]) {
            $nested = Resolve-DepartmentFromUser -User $value
...
function Upsert-BestIdentity {
    ...
    if (-not $Map.ContainsKey($normalizedId)) {
        $Map[$normalizedId] = @{
            ...
            sourceIp = $SourceIp
            department = (Normalize-Department -Value $Department)
...
foreach ($device in $devices) {
    ...
    try {
        ...
        foreach ($user in $users) {
            ...
            $department = Resolve-DepartmentFromUser -User $user
            if ([string]::IsNullOrWhiteSpace($externalId) -or [string]::IsNullOrWhiteSpace($fullName)) { continue }

            $usersRead += 1
            Upsert-BestIdentity -Map $bestByExternalId -ExternalId $externalId -FullName $fullName -SourceIp $ip -Department $department | Out-Null
        }
    }
    catch {
        $status = "USER_ERROR"
